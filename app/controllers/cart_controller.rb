class CartController < ApplicationController
  ...
  before_action :set_cart, only: [:add, :checkout]
  before_action :set_cart_item, only: [:destroy]

  # GET /cart/add/1
  def add
    @product = Product.find params[:product_id]
    @cart_item = @cart.newItem @product
    render partial: 'add_modal', locals:{cart_item:@cart_item, product: @product}, layout: nil
  end

  # POST /cart/add/1
  def create
    @product = Product.find cart_item_sizable_params[:product_id]
    @cart_item = CartItemSizable.new cart_item_sizable_params
    if @cart_item.save
